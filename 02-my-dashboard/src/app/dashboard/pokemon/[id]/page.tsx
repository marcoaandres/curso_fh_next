...
// * Para generar la meta data en una pagina dinamica y que sea generada en el servidor debemos usar generateMetadata
// aquí volvemos a utilizar la peticion getPokemon sin embargo no afecta ya que como estamos usando next solo se hace 1 vez la petcion http y se guarda en cache la segunda vez se hace la peticion pero al resultado de la cache, adicional esto se hace en la parte del servidor y el usuario no tiene problemas con el performarce
export async function generateMetadata({params}:Props): Promise<Metadata> {
  ...
  try {
    ...
    // Meta data por default si falla la generación
    return {
...
const getPokemon = async(id: string): Promise<Pokemon> => {
  ...
  //  y volver a levantar
  // podemos revisar la carpeta de .next/server/app/dashboard/pokemon aquí veremos las 151 paginas generadas
  export async function generateStaticParams() {
    
    // creacion de arreglo de 151 pokemons
  // crea un arreglo apartir de algo(por ejemplo un objeto)
  const static151Pokemons = Array.from({ length:151 }).map( (v, i) => `${ i + 1 }` );
  
  // generará de forma estatica las 151 páginas de los pokemons
